...
class sqlite3 extends storage_base
{
    ...
    protected function fetch_token_data(array $tokens)
    {
        ...
        foreach ($tokens as $token) {
            $escaped[] = $this->sqlite->escapeString($token);
        }
    ...
    protected function add_token($token, array $count)
    {
        ...
        $query->bindParam(':ch', $count[\b8\b8::KEY_COUNT_HAM], SQLITE3_INTEGER);
        $query->bindParam(':cs', $count[\b8\b8::KEY_COUNT_SPAM], SQLITE3_INTEGER);
        $query->execute();
        $query->reset();
    }
